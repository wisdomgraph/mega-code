...
def _set_span_attrs(attrs: dict) -> None:
    """Set attributes on the current tracing span (no-op when tracing is off)."""
...
@traced("ensure_user_email.try_resolve_from_profile", kind="CLIENT")
def _try_resolve_from_profile() -> str:
    ...
    try:
        ...
        _set_span_attrs({"ensure_user_email.profile_email_found": bool(email)})
        if email:
            _save_cached(email)
        return email
    except Exception:
        logger.warning("Profile email resolution failed", exc_info=True)
        _set_span_attrs({"ensure_user_email.profile_resolution_error": True})
        return ""
...
@traced("ensure_user_email.apply_to")
def _apply_to(paths: list[Path], email: str) -> int:
    ...
    _set_span_attrs(
        {
            "ensure_user_email.skills_total": len(paths),
            "ensure_user_email.skills_touched": touched,
        }
    )
...
@traced("ensure_user_email.resolve_and_apply")
def _resolve_and_apply(non_interactive: bool) -> int:
    paths = _iter_pending_skill_files()
    _set_span_attrs(
        {
            "ensure_user_email.pending_count": len(paths),
            "ensure_user_email.non_interactive": non_interactive,
        }
    )
    if not paths:
        return 0
    ...
    if not email:
        if non_interactive:
            _set_span_attrs({"ensure_user_email.skipped_non_interactive": True})
            return 0
...
@traced("ensure_user_email.set_from_env")
def _set_from_env() -> int:
    ...
    if not EMAIL_RE.match(email):
        ...
        return 1
    _save_cached(email)
    _set_span_attrs({"ensure_user_email.cached_email": True})
    return 0


@traced("ensure_user_email.apply_all_pending")
