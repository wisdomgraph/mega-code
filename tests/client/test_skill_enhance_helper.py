...
def test_apply_cli_project_dir_sets_real_project_dir(tmp_path, monkeypatch):
    ...
    assert Path(os.environ["CLAUDE_PROJECT_DIR"]) == project_dir.resolve()
...
def test_scan_project_installed_skills_uses_claude_project_dir_and_nested_metadata_author(
    ...
    plugin_dir.mkdir()
    skills_dir = project_dir / ".claude" / "skills" / "nested-author-skill"
...
""",
    ...
    assert skills[0]["path"].endswith("/real-project/.claude/skills/nested-author-skill/SKILL.md")


def test_resolve_current_project_dir_prefers_session_mapping_over_plugin_cache(
    tmp_path, monkeypatch
):
    """Plugin-cache CLAUDE_PROJECT_DIR should not override session-backed project mapping."""
    ...
    monkeypatch.setenv("CLAUDE_PROJECT_DIR", str(plugin_cache_dir))
...
def test_list_skills_uses_session_mapped_project_when_claude_project_dir_is_plugin_cache(
    ...
    fake_session_dir = tmp_path / "projects" / project_id / session_id
    skill_dir = real_project / ".claude" / "skills" / "session-mapped-skill"
...
def test_resolve_skill_repairs_project_installed_folder_name(tmp_path, monkeypatch):
    project_dir = tmp_path / "project"
    skill_dir = project_dir / ".claude" / "skills" / "legacy-folder-name"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """\
---
...
""",
    ...
    assert path == str(project_dir / ".claude" / "skills" / "canonical-skill-name" / "SKILL.md")
    assert not skill_dir.exists()


def test_resolve_skill_repairs_user_installed_folder_name(tmp_path):
    user_skills_dir = tmp_path / ".claude" / "skills"
